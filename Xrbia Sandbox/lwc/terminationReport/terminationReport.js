...
import getProject from '@salesforce/apex/terminationData.getProject';
import Wrapper from '@salesforce/apex/terminationData.Wrapper';
...
export default class TerminationReport extends LightningElement {
    ...
    exportReportData(){
        ...
        doc += '<th>Over All Connected</th>'
        doc += '<th>Terminated By Agent</th>'
        doc += '<th>Terminated By Customer</th>'
        
        this.keyOptionList.forEach(record => {    
            doc += '<tr>';
            doc += '<th>'+record.key+'</th>'; 
            doc += '<td>'+record.value.overAllConnected+'</td>'; 
            doc += '<td>'+record.value.agentHangUp+'</td>';
            doc += '<td>'+record.value.userHangup+'</td>';
        ...
        downloadElement.download = 'Termination Report.xls';
