        ...
        return {
            projects: function() {
                sforce.connection.query(
                    ...
                    " from Project__c" +
                    " where Active__c = true ", handler("projects")
                );
            },
            towers: function(proj) {
                sforce.connection.query(
                    " select Id, Name, Image_Path__c, Tower_Name__c, Total_Floors__c, Description__c, Available_Car_Parking__c, Total_Alloted_Parking__c, Total_Parking__c, No_of_Units__c, Pie_Chart_Tower_URL__c " +
                    " from Tower__c " +
                    " where Project_Name__c = '" + proj.Id + "' " +
                    " and Active__c = true ", handler("towers")
                );
            },
            floors: function(tower) {
                sforce.connection.query(
                    " select Id, Name, Description__c, Tower__r.Pie_Chart_Tower_URL__c, Floor_Number__c" +
                    " from Floor__c " +
                    " where Tower__c = '" + tower.Id + "' " +
                    " and Active__c = true ", handler("floors")
                );
            },
            properties: function(tower) {
                sforce.connection.query(
                    " select Id, Name, Description__c, Refuge__c, Blocked_by_Management__c, Total_Cost__c, Mortgage__c, Temporary_Block__c, Active__c, Property_Alloted_Through_Offer__c, House_Unit_No__c, Total_Sales_Price__c,RecordType.Name,Tower__r.Tower_Name__c,TSP__c,TSC__c," +
                    " Basic_Sale_Price__c, Allotted__c, Category__c, Discount__c, Super_Area__c, Area_Unit__c, Property_Status__c, Floor_Number__c, Unit_Type__c, Property_on_hold_for_Reallocation__c " +
                    " from Property__c " +
                    " where Floor__r.Tower__c = '" + tower.Id + "' " +
                    " order by House_Unit_No__c asc", handler("properties")
                );
            },
    ...
    .controller("floor", function($scope, $rootScope, $soql, $util) {
        ...
        $scope.$on("soql.properties", function(broadcast, args) {
            ...
            angular.forEach(args, function(p) {
                ...
                if (p.Active__c == 'true' && (p.Allotted__c == 'true' || p.Property_Alloted_Through_Offer__c == 'true')) {
                    p.$theme = 'booked';
                    p.$msg = "Booked";
