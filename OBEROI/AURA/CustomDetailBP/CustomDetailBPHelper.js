    ...
    checkPercLimit : function(component, event, helper,targetValue,keyid) {
       ...
       for(var j = 0; j < RowItemList.length; j++){
            ...
            if(RowItemList[j].Percentage_Of_Value_To_Be_Invoiced__c == undefined){
            	...
            	if(RowItemList[j].Payment_Plan_Line_Item_Name__c == 'To be paid on or before <Date>' ||
                  RowItemList[j].Payment_Plan_Line_Item_Name__c == 'Proportionate N instalments upon casting of relevant slabs.Out of which * relevant slabs have been casted and the relevant amount of Rs. * towards the Sale Price is payable as below'){
                    if(RowItemList[j] != RowItemList[keyid]){
        ...
        if(RowItemList[keyid].Payment_Plan_Line_Item_Name__c == 'To be paid on or before <Date>' ||
         RowItemList[keyid].Payment_Plan_Line_Item_Name__c == 'Proportionate N instalments upon casting of relevant slabs.Out of which * relevant slabs have been casted and the relevant amount of Rs. * towards the Sale Price is payable as below'){
